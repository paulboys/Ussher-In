...
def _refresh_meta_flags(payload: dict) -> None:
    ...
    contains_marker_focus = any(
        bool(line.get("contains_marker", False)) or marker_pattern.search(str(line.get("text_gold", "")))
        for line in lines
    )
...
def _validate_payload(payload: dict) -> list[str]:
    ...
    for line in _all_lines(payload):
        ...
        if marker_target and marker_target not in footnote_ids:
            errors.append(f"{line_id}: marker_link_target '{marker_target}' not found in footnote line_ids")
...
def index() -> str:
    ...
    if not pages:
        return "No annotation JSON files found in 08_working_scratch/phase3b/annotations", 404
...
def api_save_page(page_id: str):
    ...
    if not isinstance(incoming, dict):
        return jsonify({"ok": False, "errors": ["Request body must be JSON object"]}), 400
